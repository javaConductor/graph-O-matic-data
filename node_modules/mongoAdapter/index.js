...
(function (wu, mongoose, model, extend, logger, q) {
    ...
    var afterReadView = function(v){
        v.items = v.items.filter(function(vi){return vi.item;}).map(function(viewItem){
    ...
    var beforeSaveView = function(v){
        v.items = v.items.map(function(viewItem){
        ...
        q.when(itmP, function(itm){
           ...
           // console.log("mongoAdapter.saveItem as modelItem(" + JSON.stringify( item ) + ")");
            item.save(function ( err, i ) {
                ...
                else    logger.debug("mongoAdapter.saveItem saved(" + JSON.stringify( i ) + ")");

                return (err) ? d.reject("mongoAdapter.saveItem Error:"+err) : d.resolve(  extend({},  afterRead.item(i)) );
            });
        });
        return d.promise;
    };
...
//        f = callbackWithReport("mongoAdapter.getItems()", f);
    ...
    this.updateViewItems = function (view) {
        var d = q.defer();
        var v = new model.View(view);
        model.View.findOne({_id: view.id}, function(err, doc){
            if(err){
                d.reject(err);
            }
            else{

                view.items.map(function(vi){
                    vi.item = vi.item._id;
                })

                doc.items = view.items;
    ...
    this.xxxcreateViewItem = function (viewId, itemId, x, y) {
        var d= q.defer();
...
//        f = callbackWithReport("mongoAdapter.saveContext(" + context + "," + area + "," + JSON.stringify(types) + ")", f);
    ...
    this.saveView = function (viewP){
        ...
        q.when(viewP, function(view){
            ...
            ///TODO : Validate the item ids to be sure they are real
            logger.debug("mongoAdapter.saveView(" + JSON.stringify(view) + ")");
            var o = new model.View(beforeSave.view(view));
            o.items = [];///do not save any items initIALLY with view
